...
const handleGitHubSignup = $(async()=> {
  const { data, error } = await supabase.auth.signInWithOAuth({
    provider: 'github',
    options: {
      redirectTo: loc.url.href + "staging"
    }
  })
  console.log(data);
  console.log("----------------------------")
  ...
  const handleEmailSignup = $(async (event: any) => {
    ...
    const pwd = Math.floor(Math.random() * 99999999) +  email + timestamp;
    ...
    // Signup validation
    if(data?.user?.id) {
      message.message = "Success. Please check your email, consider looking @ ya spam folder.",
      message.status = "success"
      isLoading.value = false;
      return;
    } else {
      message.message = "There was a problem creating a user." + error?.message + error?.cause,
